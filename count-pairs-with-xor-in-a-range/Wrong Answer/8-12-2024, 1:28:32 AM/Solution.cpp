...
public:
    ...
    static string intToBinstr(int i) {
    ...
    struct Trie {
        ...
        int search(string s, string pref, int k) {
            cout << endl;
            cout << sz << endl;
            cout << s << " " << pref << endl;
            ...
            cout << mxs << " " << mns << endl;
            int mx = binstrToInt(mxs);
            int mn = binstrToInt(mns);
            cout << "MN " << mx << " " << mn << endl;
    ...
    int countPairs(vector<int>& nums, int low, int high) {
        ...
        auto lessThanK = [&](int k) { 
            ...
            int ans = 0;
            for (int i=0; i<N; i++) {
                int val = obj->search(intToBinstr(nums[i]), "", k);
                cout << "VAL " << val << endl;
                ans += val;

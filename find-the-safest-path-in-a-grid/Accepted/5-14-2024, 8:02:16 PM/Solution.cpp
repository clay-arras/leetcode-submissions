...
public:
    ...
    void fill(vector<pair<short int, short int>>& thieves) {
        ...
        while (!q.empty()) {
            auto [x, y] = q.front();
            q.pop();
            if (x + 1 < N && g[x + 1][y] == SHRT_MAX) {
                g[x + 1][y] = g[x][y] + 1;
                q.push({x + 1, y});
            }
            if (y + 1 < N && g[x][y + 1] == SHRT_MAX) {
                g[x][y + 1] = g[x][y] + 1;
                q.push({x, y + 1});
            }
            if (x - 1 >= 0 && g[x - 1][y] == SHRT_MAX) {
                g[x - 1][y] = g[x][y] + 1;
                q.push({x - 1, y});
            }
            if (y - 1 >= 0 && g[x][y - 1] == SHRT_MAX) {
                g[x][y - 1] = g[x][y] + 1;
                q.push({x, y - 1});
            }
        }
    ...
    short int lastTrue(short int lo, short int hi,
                       function<bool(short int)> f) {
        lo--;
    ...
    bool dfs(short int x, short int y, vector<bitset<MXN>>& visited,
             short int k) {
        visited[x][y] = true;
    ...
    int maximumSafenessFactor(vector<vector<int>>& grid) {
        ...
        vector<pair<short int, short int>> thieves;
        for (int i = 0; i < N; i++)
            for (int j = 0; j < N; j++)
                if (grid[i][j])
